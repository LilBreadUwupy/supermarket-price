...
<?php
    if (isset($_POST['search'])) {
        ...
        <div class="form-h1-div">
            <h1>Resultados para <span class="keywordG"> <?=$keywords ?> </span> </h1>
        </div> <?php
    } else { ?>
        <div class="form-h1-div">
            <h1> Todos los productos de <span class="keywordG"> Excelsior Gama </span> </h1>
        </div> <?php
    } ?>
...
<?php
    if (isset($_POST['search'])) {
        $keywords= $_POST['keywords'];
        $products = $conn->query("SELECT * FROM products WHERE supermarket='ExcelsiorGama' AND (product LIKE '%" . $keywords . "%') ORDER BY id asc");
    } else {
